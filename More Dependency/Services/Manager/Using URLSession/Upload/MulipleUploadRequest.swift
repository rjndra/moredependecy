...
class MultipleUploadRequest<T: Mappable, K: Mappable> : UploadRequestProtocol {
    ...
    func makeRequest(from data: RequestDataType, requestState: APIRequestState) throws -> URLRequest {
        ...
        if data.localPaths.count > 0 {
            let data = try UploadRequestHelper().convertMultipleFileData(fieldName: data.dataKey, fileUrlPaths: data.localPaths, using: boundary)
